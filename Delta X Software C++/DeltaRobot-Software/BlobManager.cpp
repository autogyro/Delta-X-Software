...
void BlobManager::UpdateNewPositionObjects(float deltaX, float deltaY)
{
	...
	for (int i = 0; i < ObjectContainer.size(); i++)
	{
		...
		emit NewUpdateObjectPosition(QString("#O_X") + QString::number(i), obj.center.x);
		emit NewUpdateObjectPosition(QString("#O_Y") + QString::number(i), obj.center.y);
		emit NewUpdateObjectPosition(QString("#O_A") + QString::number(i), angle);
	}	
}
